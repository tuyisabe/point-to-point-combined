...
import { Text, View, StyleSheet, Dimensions, FlatList, Modal, TouchableHighlight, Switch, Image, Platform, Linking, TouchableOpacity } from 'react-native';
    ...
    const onPressAccept = (item) => {
        ...
        if((settings && settings.imageIdApproval && auth.profile.verifyId && auth.profile.verifyIdImage) || (settings && !settings.imageIdApproval)){
            if(!settings.negativeBalance && !settings.disable_cash && (wallet_balance <= 0 || (wallet_balance > 0 && wallet_balance < item.convenience_fees)) && item.payment_mode === 'cash'){
                ...
                if(parseFloat(amount[item.id]) > 0){
                    const  profile =  auth.profile;
                    let convenience_fees = item.commission_type == 'flat'? parseFloat(item.commission_rate) :  (parseFloat(amount[item.id]) * parseFloat(item.commission_rate)/100);
                    let obj = {};
                    ...
                    obj.bidPrice = amount[item.id];
                    obj.trip_cost = convenience_fees + parseFloat(amount[item.id]);
                    obj.convenience_fees = convenience_fees;
                    obj.driver_share = parseFloat(amount[item.id]);
                    obj.car_image =  profile.car_image ? profile.car_image : "";
        ...
        <View style={styles.mainViewStyle}>
            <FlatList
                data={auth.profile && auth.profile.uid && auth.profile.driverActiveStatus ?
                    (auth.profile.queue ? activeBookings : tasks) : []}
